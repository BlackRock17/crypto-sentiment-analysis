...
@router.get("/tweets/status/{tweet_id}", status_code=status.HTTP_200_OK, response_model=dict)
async def check_tweet_status(tweet_id: str):
    """
    Check the status of a tweet processing.
    """
    try:
        ...
        db_tweet = repo.get_tweet_by_id(tweet_id)
        ...
        if db_tweet:
            ...
            return {
                "status": "pending",
                "message": "Tweet still being processed",
                "tweet_id": tweet_id
            }

    except Exception as e:
        logger.error(f"Error checking tweet status: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to check tweet status: {str(e)}"
        )
